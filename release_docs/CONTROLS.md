...
| Action | Keyboard | Standard controller |
|---|---|---|
| Left / right | A / D | D-pad or left stick |
| Jump / crouch | W / S | D-pad or left stick |
| Light punch — LP | U | X / west face |
| Medium punch — MP | I | Y / north face |
| Heavy punch — HP | O | Right shoulder |
| Light kick — LK | J | A / south face |
| Medium kick — MK | K | B / east face |
| Heavy kick — HK | L | Left shoulder |
| Two punches — PP | P | Left-stick click |
| Two kicks — KK | Semicolon | Right-stick click |

PP and KK emit simultaneous button chords. They do not perform a motion for you. Press LP + LK to throw or tech a normal throw; MP + MK performs the universal leap overhead. Forward + HP is the command normal. Back + HP + HK uses an equipped gambit; HP + HK alone is cosmetic.

Opposing horizontal directions resolve to neutral; opposing vertical directions also resolve to neutral. Stick deadzone is adjustable. No input device is automatically assigned to both fighters.

## Movement and defense

Walk by holding left/right. Double-tap toward or away to dash. Jump toward, away or straight up. There is no double jump or air dash.

Hold away to guard high/mid attacks. Hold down-away to guard lows. Air blocking is unavailable. To parry, return to neutral and tap toward for high/mid or airborne contact, or down for a low attack. Multihit attacks require separate parries. A red parry is a tighter parry from blockstun.
...
| Action | Keyboard shortcut | Controller route |
|---|---|---|
| Pause / resume | F1 | Start → Resume |
| Single frame | Period while paused | Start → Single frame |
| Reset drill | F2 | Start → Reset drill |
| Change dummy | F3 | Start → Dummy |
| Next drill | F4 | Start → Next drill |
| Show collision boxes | F5 | Start → Hitboxes |
