# Historical direct-spend baseline — superseded by Shop-Only v2

The original notes below are retained as history. Their rules, results, identifiers and reproduction commands belong only to the recorded prior candidate. Relative evidence links originally resolved from `release_docs/BALANCE_NOTES.md` in that baseline checkout.
