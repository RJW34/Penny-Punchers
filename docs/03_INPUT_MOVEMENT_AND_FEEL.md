...
Contextual priority: throw tech where valid; selected super; EX special; ordinary special; throw; leap overhead; leased gambit/taunt; command normal; proximity normal; normal. In hitstun only legal buffering/defense applies; priorities cannot escape it. Paid recognition has two stages: syntax then legal action transition and affordability. If a legal paid command lacks credits or breaches the locked reserve, consume that occurrence, display a small rejection cue, and do not charge, substitute a regular move, queue until payday, or freeze the player. A command recognized during an illegal cancel does not magically become legal.
