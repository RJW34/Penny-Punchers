...
def main():
    p=argparse.ArgumentParser();p.add_argument('mode',choices=['match','free-kit','ui','showcase','network','linux','windows-headless','linux-headless','free-kit-headless']);p.add_argument('--movie',action='store_true');p.add_argument('--fps',type=int,default=30);p.add_argument('--label');p.add_argument('--profile',action='store_true');a=p.parse_args()
    ...
    for seat in range(2 if a.mode=='network' else 1):
        ...
        if a.movie and a.mode not in ['ui','showcase']:engine+=['--resolution','960x540']
    ...
    for child,log,folder,start,cmd in jobs:
        ...
        log.write('\nEXIT '+str(code)+'\n');log.close()
        name={'ui':'controller-menu-flow.json','showcase':'combat-visual-showcase.json','network':'native-network-result.json'}.get(a.mode,'runtime-result.json')
