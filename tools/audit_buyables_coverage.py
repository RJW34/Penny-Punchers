"""Bind every selected action node to substantive current conformance evidence.
...
'buy_r_a3_overtime':('buy_route_overtime_wallet_timer','1500 exact fee,240-tick install, all8 HIT-only graph edges; separate freeze/interruption suite.')}
ACTORS={
...
'buy_v_a3_prism':('prism_exact_cost_occupancy_snapshot_expiry','Exact threshold,field occupancy/lifetime/snapshot; parry/reflection/destruction and paid recast fixtures.')}
def main():
 out=[];object_path='reports/evidence/buyable-objects-published/buyable-object-tests.json';objects=read(object_path);object_pass={x['name'] for x in objects['results'] if x['passed']}
 for name,directory in [('core','data'),('full','data/rulesets/buyables_full')]:
  reportpath=f'reports/evidence/buyables-{name}-regression/core-conformance.json';report=read(reportpath);passed={x['id'] for x in report['results'] if x['pass']};direct={(x['fighter'],x['move']):x for x in report['actionCoverage']};nodes=[]
  for fp in sorted((ROOT/directory/'fighters').glob('*.json')):
   ...
   for move in fighter['moves']:
    ...
    if key in direct:
     ...
     sources=[reportpath,f'reports/evidence/buyables-{name}-regression/action-replays.training.json'];kind='reconstructed training action effect';description=f"Observed damage={d['damage']}, displacement={d['displacement']}; exact startup/input/event/final-state reconstruction."
     ...
     if move['id'] in ROUTES:sources.append(f'reports/evidence/buyables-{name}-regression/audit-legal-traces.json')
     if move['id'] in ('buy_r_s4_riposte','buy_v_t3_descending_heel'):
      item='buy_r_s4_counter' if 'riposte' in move['id'] else move['id'];trace=next((ROOT/'reports/evidence/buyables-rental-pilot/traces').glob(f'rental-{item}-pressure-s1-seat0-lease.json.gz'))
     ...
     case,description=OBJECTS[move['id']];assert case in object_pass;assert objects['contentHash']==report['content_hash'];sources=[object_path,'reports/evidence/buyable-objects-published/buyable-object-traces.jsonl','src/StrikeLedger.NetworkLab/BuyableObjectTests.cs'];kind='passed object arbitration/lifecycle assertions and per-tick event trace'
  ...
  out.append(dict(ruleset=name,content_hash=report['content_hash'],core_assembly_id=report['core_assembly_id'],action_nodes=len(nodes),covered_nodes=len(nodes),nodes=nodes,not_applicable_historical_cases=report['notApplicable']))
 paths=sorted({p for trial in out for node in trial['nodes'] for p in node['evidence']});result=dict(format='penny-buyables-action-effect-coverage-v1',trials=out,artifacts=[dict(path=p,sha256=sha(p)) for p in paths],limits=['Coverage proves the specified mechanical effects in bounded fixtures; it does not prove every defense interaction, optimal purchase value, native sprite alignment or human competitive fairness.','A neutral-entry demonstration is not used as sole effect proof. Derived nodes are verified through their owning route or catch.','Core and full trials are different content identities. Historical98-action evidence remains separate.'])
 destination=ROOT/'reports/evidence/buyables-action-coverage.json';destination.write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8');print(json.dumps([dict(ruleset=x['ruleset'],covered=x['covered_nodes'],total=x['action_nodes']) for x in out]))
