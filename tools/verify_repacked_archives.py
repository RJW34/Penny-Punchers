...
def main():
    ...
    prior=json.loads((ROOT/'reports/evidence/package-verification/result.json').read_text())
    ...
    for platform in ['windows','linux']:
        ...
        with zipfile.ZipFile(archive) as z:
            ...
            manifest=z.read('StrikeLedger/SHA256SUMS.txt').decode().splitlines()
            for row in manifest:
                sha,name=row.split('  ',1);assert digest(z.read('StrikeLedger/'+name))==sha,name
            for name,sha in expected.items():
                assert digest(z.read('StrikeLedger/'+name))==sha and digest((extracted/name).read_bytes())==sha,name
            assert z.read('StrikeLedger/RELEASE_CANDIDATE.json')==(ROOT/'reports/RELEASE_CANDIDATE.json').read_bytes()
            for file in (ROOT/'release_docs').rglob('*'):
                if file.is_file():assert z.read('StrikeLedger/docs/'+file.relative_to(ROOT/'release_docs').as_posix())==file.read_bytes()
            assert z.read('StrikeLedger/docs/ACCEPTANCE_RESULTS.json')==(ROOT/'reports/ACCEPTANCE_RESULTS.json').read_bytes()
            if platform=='linux':assert z.getinfo('StrikeLedger/StrikeLedger.x86_64').external_attr>>16&0o111
    ...
    (ROOT/'reports/evidence/final-archive-integrity.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
