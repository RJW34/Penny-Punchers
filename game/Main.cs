...
{
    readonly Color Ink=new("101921"),Cream=new("eee7d7"),Muted=new("98a9ad"),Gold=new("ecac52"),Cyan=new("72d7d6");
    ...
    public override void _PhysicsProcess(double delta){if(quitting)return;TickUiSmoke(delta);TickCombatShowcase(delta);TickNetworkEvidence(delta);int ticks=smoke&&DisplayServer.GetName()=="headless"?60:1;for(int i=0;i<ticks;i++)TickShell(delta);}
    ...
    public override void _Draw()
    {
        ...
        if(font==null)return;if(sim!=null&&(screen is "fight" or "pause")){
            DrawRect(new Rect2(0,0,1280,153),new Color(.025f,.045f,.06f,.96f));DrawLine(new(0,153),new(1280,153),new Color("3b4c50"),2);
            for(int i=0;i<2;i++){var p=sim.Players[i];float x=i==0?42:736;Color c=i==0?Gold:Cyan;DrawString(font,new(x,34),$"{p.FighterId.ToUpperInvariant()} / {p.ScoreHalfPoints/2f:0.#} PTS",HorizontalAlignment.Left,-1,23,Cream);DrawRect(new Rect2(x,46,502,21),new Color("2d3437"));DrawRect(new Rect2(i==0?x:x+502*(1-p.Health/1000f),46,502*p.Health/1000f,21),c);DrawString(font,new(x,97),$"{p.Credits:N0} CR",HorizontalAlignment.Left,-1,25,c);DrawString(font,new(x+186,96),$"EX 300 {(p.Credits-p.ReserveFloor>=300?"READY":"LOW FUNDS")}",HorizontalAlignment.Left,-1,14,Muted);DrawString(font,new(x,127),$"ART {art[i]+1} / {900+art[i]*300} CR    RESERVE {p.ReserveFloor}    STUN {p.Stun}",HorizontalAlignment.Left,-1,13,Muted);}
            DrawLeaseSummary();DrawString(font,new(592,63),$"{Math.Max(0,sim.TimerTicks+59)/60:00}",HorizontalAlignment.Left,-1,47,Cream);DrawString(font,new(586,98),$"ROUND {sim.RoundId}",HorizontalAlignment.Left,-1,13,Gold);
            string banner=sim.Phase switch{MatchPhase.Reveal=>"LOADOUTS LOCKED",MatchPhase.Countdown=>"GET READY",MatchPhase.PendingResult=>peer==null?"K.O.":"CONFIRMING RESULT",MatchPhase.RoundResult=>"ROUND SETTLED",MatchPhase.MatchOver=>"MATCH COMPLETE",_=>""};if(banner!=""){DrawRect(new Rect2(352,270,576,104),new Color(.03f,.055f,.07f,.85f));DrawString(font,new(397,334),banner,HorizontalAlignment.Center,486,36,Cream);}
            if(mode=="training"){DrawRect(new Rect2(0,609,1280,111),new Color(.025f,.045f,.06f,.94f));DrawString(font,new(24,635),$"LAB / {(paused?"PAUSED":"LIVE")}  DUMMY: {lab?.DummyMode.ToString()??trainingMode}  {lab?.Drill.Title}",HorizontalAlignment.Left,-1,17,Gold);DrawString(font,new(24,666),"F1 pause  . frame  F2 reset  F3 dummy  F4 drill  F5 boxes  ESC menu",HorizontalAlignment.Left,-1,16,Cream);DrawString(font,new(24,698),$"P1 {lastInputs[0].Direction}/{lastInputs[0].Held} {sim.Players[0].ActionId} f{sim.Players[0].ActionFrame} | P2 {lastInputs[1].Direction}/{lastInputs[1].Held}",HorizontalAlignment.Left,-1,15,Muted);for(int i=0;i<inputHistory[0].Count;i++)DrawString(font,new(24,198+i*23),inputHistory[0].ElementAt(i),HorizontalAlignment.Left,-1,14,Muted);}else {DrawRect(new Rect2(0,672,1280,48),new Color(.025f,.045f,.06f,.96f));DrawString(font,new(42,702),"FIRST TO 5 POINTS / MAX 9 ROUNDS                              ESC / START  pause & move list",HorizontalAlignment.Left,-1,14,Muted);if(peer!=null)DrawString(font,new(40,658),$"PRIVATE / DELAY {peer.Rollback.InputDelay}f / ROLLBACK {peer.Rollback.RollbackCount} / {(peer.Rollback.IsStalled?"WAITING FOR INPUT":"CONNECTED")}",HorizontalAlignment.Left,-1,14,Cyan);}
        DrawLabDiagnostics();DrawReplayOverlay();}if(toastTime>0){DrawRect(new Rect2(350,158,580,43),new Color(.05f,.085f,.1f,.98f));DrawString(font,new(369,187),toast,HorizontalAlignment.Center,542,19,Gold);}
    }
    ...
    public override void _ExitTree(){Input.JoyConnectionChanged-=OnJoyChanged;peer?.Dispose();evidenceLog?.Dispose();}
