...
public partial class Main : Node2D
{
    ...
    string stageId="foundry";
    string FighterName(string id)=>content.Fighters.TryGetValue(id,out var definition)?definition.DisplayName:id;
    string[] fighter={"rook","vale"};int[] art={0,0};bool[] ready={false,false};
    ...
    string trainingMode="Stand";int dummyMode,drillIndex;readonly string[] drills={"Free practice","High parry: tap toward at impact","Low parry: tap down at impact","Throw tech: LP + LK","Confirm: LP > MP > special","Kara: forward HP > LP+LK","License: buy EX, then QCF + PP"};
    ...
    {
        GetTree().AutoAcceptQuit=false;font=UiTypography.Body;settings=GameSettings.Load();settings.Apply();input=new(settings);arena=new ArenaView();AddChild(arena);arena.SetVolumes(settings.Master,settings.Music,settings.Sfx);
        var layer=new CanvasLayer();AddChild(layer);ui=new Control{Theme=new Theme{DefaultFont=UiTypography.Body}};ui.SetAnchorsAndOffsetsPreset(Control.LayoutPreset.FullRect);layer.AddChild(ui);
        ...
        try{dataPath=ExtractContent();LoadContentRuleset(Arg("--ruleset","buyables_core"));}
    ...
    void StartMatch(){if(mode=="training"){StartLab();return;}var config=new MatchConfig{Fighter0=fighter[0],Fighter1=fighter[1],Super0="",Super1="",SessionId="local",StageId=stageId,Training=false};sim=new Simulation(content,config);bot0=new(0,BotMode.Adaptive,17);bot1=new(1,cpuPolicy,31);recorder=new ReplayRecorder(content,config);lastWallet=sim.Players.Select(p=>p.Credits).ToArray();lastHealth=sim.Players.Select(p=>p.Health).ToArray();paused=false;Preparation();}
    ...
    public override void _Draw()
    {
        ...
        if(font==null)return;if(sim!=null&&(screen is "fight" or "pause")){
            ...
            if(mode=="training"){DrawStyleBox(UiArtFrame("hud"),new Rect2(0,672,1280,48));DrawString(font,new(24,691),$"LAB / {(paused?"PAUSED":"LIVE")}  DUMMY: {lab?.DummyMode.ToString()??trainingMode}  {lab?.Drill.Title}",HorizontalAlignment.Left,1232,14,Gold);DrawString(font,new(24,713),"F1 pause  . frame  F2 reset  F3 dummy  F4 drill  F5 boxes  ESC menu",HorizontalAlignment.Left,-1,16,Cream);for(int i=0;i<inputHistory[0].Count;i++)DrawString(font,new(42,224+i*23),inputHistory[0].ElementAt(i),HorizontalAlignment.Left,-1,14,Muted);}else {DrawStyleBox(UiArtFrame("hud"),new Rect2(-12,672,1304,60));DrawString(font,new(42,702),"FIRST TO 5 POINTS / MAX 9 ROUNDS                              ESC / START  pause & move list",HorizontalAlignment.Left,-1,14,Muted);if(peer!=null)DrawString(font,new(40,658),$"PRIVATE / DELAY {peer.Rollback.InputDelay}f / ROLLBACK {peer.Rollback.RollbackCount} / {(peer.Rollback.IsStalled?"WAITING FOR INPUT":"CONNECTED")}",HorizontalAlignment.Left,-1,14,Cyan);}
    ...
    public override void _Process(double delta){RuntimeProfiler.MarkFrame(screen+":"+(sim?.Phase.ToString()??"none"),sim?.Tick??-1);RecordRenderedFrame(delta);if(pendingCapture!=""&&captureDelay-->0)return;if(pendingCapture!=""){var path=System.IO.Path.Combine(evidenceDir,pendingCapture+".png");using(var captureImage=GetViewport().GetTexture().GetImage())captureImage.SavePng(path);GD.Print("CAPTURE "+path);pendingCapture="";}}
    public override void _Input(InputEvent e){input?.ObserveEvent(e);if(RouteControlInput(e))return;if(computerEvidence&&e is InputEventKey k&&k.Pressed&&!k.Echo)GD.Print($"KEY {k.Keycode}/{k.PhysicalKeycode} SCREEN {screen}");if(e is InputEventKey shot&&shot.Pressed&&(shot.Keycode==Key.F12||shot.PhysicalKeycode==Key.F12)){Capture("manual-"+DateTime.Now.ToString("HHmmss"));GetViewport().SetInputAsHandled();}}
