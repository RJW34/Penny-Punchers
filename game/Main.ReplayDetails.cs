...
public partial class Main
{
    void DrawReplayDetails()
    {
        ...
        if(showReplayReceipts){var receipts=activeReplayRecord.Commands.Take(Math.Max(0,replay.CommandIndex)).SelectMany(c=>c.Debits).TakeLast(4).ToArray();if(receipts.Length>0){DrawRect(new Rect2(26,462,490,137),new Color(.025f,.045f,.06f,.94f));DrawString(font,new(40,486),"CONFIRMED ACTION DEBITS",HorizontalAlignment.Left,-1,13,Cream);for(int i=0;i<receipts.Length;i++){var d=receipts[i];DrawString(font,new(40,511+i*24),$"{d.Tick} / P{d.Seat+1} {d.Move} −{d.Amount} CR",HorizontalAlignment.Left,-1,14,d.Seat==0?Gold:Cyan);}}}
    }
}
